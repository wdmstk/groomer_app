...
### in_progress
1. `TASK-453` ユーザーマニュアル再構築（通常/本部/開発の実運用網羅）
2. `TASK-452` UI改善β導線の追加（メニュー/予約/カレンダー/運用設定）
3. `TASK-450` スタッフ管理機能拡張（シフト管理・勤務管理）
4. `TASK-449` ページ/APIルート網羅テストの一括強化（大塊PR運用）
5. `TASK-448` 顧客管理βページ改善（会員証URLカード配色統一）
6. `TASK-408` POS機能導入計画（要件定義〜実装）
7. `TASK-POS-006` 受入試験・移行・運用ドキュメント整備（`TASK-408`配下）
...
### done
1. `TASK-451` 主要画面UI/導線の統一改善（改善メモ反映）（2026-04-18）
2. `TASK-447` サイドバー見た目改善（カテゴリ名とメニューの視認性分離）（2026-04-06）
3. `TASK-446` `.tsx` 向けVitest導入とUIコンポーネントテスト追加（2026-04-05）
4. `TASK-445` テスト品質/実装原則の運用ルール明文化とガード導入（2026-04-05）
5. `TASK-444` 店舗設定タブ再編と予約カレンダー営業時間連動（2026-04-05）
6. `TASK-443` 予約カレンダーの時間軸横向き化（2026-04-05）
7. `TASK-442` 店舗別決済アカウント接続と通常決済のStripe/KOMOJU対応（2026-04-05）
8. `TASK-441` 会員証URLのセルフ再取得導線（期限切れ時）（2026-04-05）
9. `TASK-440` 会員証URL有効期限の可変TTL設定（30/90/180日）（2026-04-05）
10. `TASK-439` 会員証ポータルからの空き枠待ち自己登録導線追加（2026-04-04）
11. `TASK-438` 顧客管理・ペット管理の統合ページ仕様策定（2026-04-02）
12. `TASK-437` 日誌機能（非破壊追加）仕様策定と実装管理（2026-03-31）
13. `TASK-436` 予約管理の同意書ステータスバッジと状態別導線追加（2026-03-31）
14. `TASK-435` 電子同意書管理のタブ分割・履歴PDF参照/削除・署名導線強化（2026-03-31）
15. `TASK-434` 既存店舗向け同意書テンプレ更新SQL作成（2026-04-06）
16. `TASK-433` 施術同意書標準テンプレート完全版差し替え（2026-03-29）
17. `TASK-432` 施術同意書の表示体裁改善とPDF2ページ化（2026-03-29）
18. `TASK-431` 予約管理起点の同意書作成導線とPDF作成強化（2026-03-29）
19. `TASK-430` 電子同意書管理のレイアウト統一・テンプレ本文再利用・PDF証跡強化（2026-03-28）
20. `TASK-429` アプリ名/会社名の全ページ反映（2026-03-28）
21. `TASK-428` 電子同意書PDF文字化け修正（日本語対応）（2026-03-27）
22. `TASK-427` 電子同意書Storageバケット不足エラー修正（2026-03-27）
23. `TASK-426` 電子同意書テンプレ差し込み表示とプレビュー実装（2026-03-27）
24. `TASK-425` 電子同意書の署名URL再取得導線追加（2026-03-27）
25. `TASK-424` 電子同意書テンプレートUIの用語/入力改善（2026-03-27）
26. `TASK-423` 電子同意書テンプレート標準初期化（店舗作成時の自動投入）（2026-03-27）
27. `TASK-422` 電子同意書（施術同意書）機能追加（2026-03-27）
28. `TASK-421` 競合LPベンチマーク整合のLP改修（2026-03-26）
29. `TASK-420` dev課金操作の安全運用化（2026-03-22）
30. `TASK-419` 法務・規約コンプライアンス是正（2026-03-22）
31. `TASK-418` オプション課金確定ゲート整備（2026-03-22）
32. `TASK-416` サイドバー改善（2026-03-21）
33. `TASK-415` 動画カルテ + AIプラン拡張（非破壊導入）
34. `TASK-414` 顧客LTV分析
35. `TASK-413` AIタグ解析ジョブのRLS修正（2026-04-06）
36. `TASK-412` AIタグ活用導線の改善（2026-04-06）
37. `TASK-411` 写真カルテのAIタグ付け
38. `TASK-410` LINEの自動マーケ
39. `TASK-409` 事前決済
40. `TASK-407` 開発環境で課金なしのプラン/オプション切替
41. `TASK-406` devサブスク保存後404修正
42. `TASK-405` タスク/ブランチ運用ガード追加とStorage設定導線整理
43. `TASK-404` HP本部運用にホテルメニュー向けテンプレ配信を追加
44. `TASK-403` ペット管理からQR機能を完全削除
45. `TASK-402` appointments更新後POST転送不具合修正
46. `TASK-401` 統合会計（Invoice方式）（2026-04-06）
47. `TASK-POS-007` POS一本化（トリミング/ホテル会計統合）（`TASK-408`配下）
48. `TASK-POS-005` レジ開閉局・日次締め実装（`TASK-408`配下）
49. `TASK-POS-004` 在庫連動（自動出庫/返品戻し）実装（`TASK-408`配下）
50. `TASK-POS-003` POS会計画面（MVP）実装（`TASK-408`配下）
51. `TASK-POS-002` データモデル・API契約設計（`TASK-408`配下）（2026-04-06）
52. `TASK-POS-001` 要件定義・業務フロー確定（`TASK-408`配下）（2026-04-06）

## 正式タスク詳細（Task ID採番済み）

## ユーザーマニュアル再構築（通常/本部/開発の実運用網羅）
- Task ID: `TASK-453`
- ブランチ: `feat/TASK-453-user-manual-plan`
- 現在の作業ブランチ: `feat/TASK-453-user-manual-plan`
- ステータス: `in_progress`
- 概要:
  - 店舗実運用を前提に、`通常` `本部` `開発` の3系統マニュアルを再構築するためのTODOを定義する。
  - 初版は `全機能` を対象とし、`アプリ内マニュアル` として提供する。
  - マニュアルは原則 `画面スクリーンショット付き` で作成する。
  - 記述粒度は `新人向け（手順詳細）` を基準とし、本部の `承認/監査観点` を必須で含める。
- TODO（マニュアル作成タスク）:
  - [ ] 全機能の棚卸し（画面・API連携・権限制御・オプション分岐）を `通常/本部/開発` でマッピング
  - [ ] `単店舗運用` と `複数店舗（本部統制）運用` の両シナリオを同等粒度で定義
  - [ ] 3ロール別に「日次/週次/月次/例外対応」の業務シナリオを作成
  - [ ] 各シナリオの操作手順を新人向けに詳細化（事前条件・操作・期待結果・失敗時対処）
  - [ ] 各手順に対応する確認/判断項目を定義（OK条件、差し戻し条件、エスカレーション条件）
  - [ ] 画面スクリーンショット取得計画を作成（取得対象、更新ルール、差し替え責任者）
  - [ ] 入力項目辞書を整備（必須/任意、形式、業務上の意味、誤入力時影響、修正方法）
  - [ ] 本部向け承認/監査フローを定義（承認者、証跡、監査頻度、異常時対応）
  - [ ] 開発向け運用手順を定義（設定変更、障害切り分け、ロールバック、問い合わせ導線）
  - [ ] エラー/障害時の一次切り分けと連絡フロー（通常→本部→開発）を標準化
  - [ ] アプリ内マニュアル実装仕様を確定（配置、導線、検索、章構成、アクセス権）
  - [ ] リリース後の更新運用を定義（変更履歴、版管理、改訂責任者、レビュー手順）
- 要確認事項（確定後にTODOへ反映）:
  - [x] マニュアルの提供形態（アプリ内）
  - [x] 1次リリース対象範囲（全機能）
  - [x] 店舗運用の前提（単店舗/複数店舗を両方対象）
  - [x] 記述粒度（新人向け詳細手順）
  - [x] 本部向けに必要な監査/承認フロー（必須）
  - [x] スクリーンショット方針（画面スクショありき）

## UI改善β導線の追加（メニュー/予約/カレンダー/運用設定）
- Task ID: `TASK-452`
- ブランチ: `feat/TASK-452-ui-improvement`
- 現在の作業ブランチ: `feat/TASK-452-ui-improvement`
- ステータス: `in_progress`
- 概要:
  - 非破壊移行として `メニュー管理（β）` と `予約管理（β）` を新設し、検証完了後に旧導線を削除する。
  - ホテル関連UIは `hotel_option_enabled`（実効値）有効時のみ表示する。
- 影響範囲:
  - `TASKS.md`
  - `groomer_app/src/app/service-menus*`
  - `groomer_app/src/app/appointments*`
  - `groomer_app/src/app/hotel*`
  - `groomer_app/src/app/settings*`
  - 共通ナビゲーション/タブUI、関連API利用箇所、Vitest/E2E
- 完了条件:
  - `メニュー管理（β）` が顧客業務カテゴリに追加され、`トリミングメニュー` / `ホテルメニュー` をタブ切替で利用できること
  - `予約管理（β）` が追加され、`トリマー予約` / `ホテル予約` をタブ切替で利用できること
  - カレンダーでトリマー予約/ホテル予約を同時表示でき、`全件/トリマーのみ/ホテルのみ` フィルターが機能すること
  - ペットホテル運用設定が店舗運用設定へ移設され、ホテルオプション有効時のみ表示されること
  - 旧 `施術メニュー管理` / 旧 `予約管理` / 旧 `ペットホテル管理` のサイドバー導線が削除され、β導線へ集約されること
  - Vitest/E2E/lint が通過し、回帰がないことを確認できること
- 進捗:
  - [x] ブランチ作成・切替（`feat/TASK-452-ui-improvement`）
  - [x] `TASKS.md` へ正式タスク登録（`in_progress`）
  - [x] 実装ステップ定義（この項目）
  - [x] Step 1: ホテルオプション判定の共通化（ナビ/タブ/設定表示で同一判定を使用）
  - [x] Step 2: `メニュー管理（β）` 新設（顧客業務カテゴリ）
  - [x] Step 3: `メニュー管理（β）` に `トリミングメニュー` / `ホテルメニュー` タブ追加（ホテルタブはオプション有効時のみ）
  - [x] Step 4: `予約管理（β）` 新設
  - [x] Step 5: `予約管理（β）` に `トリマー予約` / `ホテル予約` タブ追加（ホテルタブはオプション有効時のみ）
  - [x] Step 6: カレンダー統合表示（トリマー予約+ホテル予約）と表示フィルター追加
  - [x] Step 7: ペットホテル運用設定を店舗運用設定へ移設（オプション有効時のみ表示）
  - [x] Step 8: 既存導線を残したまま回帰確認（非破壊移行）
  - [x] Step 9: Vitest/E2E/lint 実施と結果反映
  - [x] Step 10: β画面UI微調整（重複タイトル/内部タブ削除、予約βトップへカレンダー移設、ホテル一覧の見た目統一）
  - [x] Step 11: 設定画面UI/導線調整（ホテル営業時間連動、公開枠ルールとアラート閾値のカード分離、通知設定リンク整理、店舗追加レイアウト統一）
  - [x] Step 12: 旧導線整理（サイドバーから `施術メニュー管理` / `予約管理` / `ペットホテル管理` を削除し、関連マニュアル導線をβ導線へ更新）
  - [x] Step 13: 正式導線化（`/menu-management`・`/reservation-management` へ統一）と旧ページ本体の完全廃止（旧3ページと `beta` ルートを削除）
...
- 参照ドキュメント:
  - `メモ/改善メモ.md`
- 影響範囲:
  - `groomer_app/src/app/dashboard*`、`groomer_app/src/app/ops/today*`、`groomer_app/src/app/customers*`
  - `groomer_app/src/app/appointments*`、`groomer_app/src/app/medical-records*`、`groomer_app/src/app/journal*`
  - `groomer_app/src/app/hotel*`、`groomer_app/src/app/inventory*`、`groomer_app/src/app/settings*`
  - 関連 UI コンポーネント、Vitest/E2E、運用ドキュメント
- リスク:
  - 画面横断で変更範囲が広く、既存運用UIの回帰が発生しやすい
  - ダークテーマ対応の不足が残ると視認性問題が断続的に再発する
  - タブ構成の統一で既存導線が変わり、利用者の認知コストが一時的に増える
- 完了条件:
  - 改善メモ記載の全項目に対して、仕様確定・実装・検証結果が説明可能な状態
  - ダークテーマでの可読性不具合が主要画面で再現しないこと
  - タブ/レイアウト統一方針が各対象画面へ反映されること
  - Vitest/E2E/lint を通過し、必要なドキュメント更新が完了していること
- 進捗:
  - [x] Step 1: 全体方針策定（ダークテーマ配色基準、タブ/レイアウト統一基準）
...
- 現在の作業ブランチ: `test/TASK-449-closure`
- ステータス: `done`
