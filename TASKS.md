...
### done
1. `TASK-443` 予約カレンダーの時間軸横向き化（2026-04-05）
2. `TASK-442` 店舗別決済アカウント接続と通常決済のStripe/KOMOJU対応（2026-04-05）
3. `TASK-441` 会員証URLのセルフ再取得導線（期限切れ時）（2026-04-05）
4. `TASK-440` 会員証URL有効期限の可変TTL設定（30/90/180日）（2026-04-05）
5. `TASK-439` 会員証ポータルからの空き枠待ち自己登録導線追加（2026-04-04）
6. `TASK-438` 顧客管理・ペット管理の統合ページ仕様策定（2026-04-02）
7. `TASK-437` 日誌機能（非破壊追加）仕様策定と実装管理（2026-03-31）
8. `TASK-436` 予約管理の同意書ステータスバッジと状態別導線追加（2026-03-31）
9. `TASK-435` 電子同意書管理のタブ分割・履歴PDF参照/削除・署名導線強化（2026-03-31）
10. `TASK-433` 施術同意書標準テンプレート完全版差し替え（2026-03-29）
11. `TASK-432` 施術同意書の表示体裁改善とPDF2ページ化（2026-03-29）
12. `TASK-431` 予約管理起点の同意書作成導線とPDF作成強化（2026-03-29）
13. `TASK-430` 電子同意書管理のレイアウト統一・テンプレ本文再利用・PDF証跡強化（2026-03-28）
14. `TASK-429` アプリ名/会社名の全ページ反映（2026-03-28）
15. `TASK-428` 電子同意書PDF文字化け修正（日本語対応）（2026-03-27）
16. `TASK-427` 電子同意書Storageバケット不足エラー修正（2026-03-27）
17. `TASK-426` 電子同意書テンプレ差し込み表示とプレビュー実装（2026-03-27）
18. `TASK-425` 電子同意書の署名URL再取得導線追加（2026-03-27）
19. `TASK-424` 電子同意書テンプレートUIの用語/入力改善（2026-03-27）
20. `TASK-423` 電子同意書テンプレート標準初期化（店舗作成時の自動投入）（2026-03-27）
21. `TASK-422` 電子同意書（施術同意書）機能追加（2026-03-27）
22. `TASK-421` 競合LPベンチマーク整合のLP改修（2026-03-26）
23. `TASK-420` dev課金操作の安全運用化（2026-03-22）
24. `TASK-419` 法務・規約コンプライアンス是正（2026-03-22）
25. `TASK-418` オプション課金確定ゲート整備（2026-03-22）
26. `TASK-416` サイドバー改善（2026-03-21）
27. `TASK-415` 動画カルテ + AIプラン拡張（非破壊導入）
28. `TASK-414` 顧客LTV分析
29. `TASK-411` 写真カルテのAIタグ付け
30. `TASK-410` LINEの自動マーケ
31. `TASK-409` 事前決済
32. `TASK-407` 開発環境で課金なしのプラン/オプション切替
33. `TASK-406` devサブスク保存後404修正
34. `TASK-405` タスク/ブランチ運用ガード追加とStorage設定導線整理
35. `TASK-404` HP本部運用にホテルメニュー向けテンプレ配信を追加
36. `TASK-403` ペット管理からQR機能を完全削除
37. `TASK-402` appointments更新後POST転送不具合修正
38. `TASK-POS-003` POS会計画面（MVP）実装（`TASK-408`配下）
39. `TASK-POS-004` 在庫連動（自動出庫/返品戻し）実装（`TASK-408`配下）
40. `TASK-POS-005` レジ開閉局・日次締め実装（`TASK-408`配下）
41. `TASK-POS-007` POS一本化（トリミング/ホテル会計統合）（`TASK-408`配下）
