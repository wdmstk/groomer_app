...
### in_progress
1. `TASK-438` 顧客管理・ペット管理の統合ページ仕様策定（2026-04-02）
2. `TASK-437` 日誌機能（非破壊追加）仕様策定と実装管理（2026-03-31）
3. `TASK-434` 既存店舗向け同意書テンプレ更新SQL作成（2026-03-29・再オープン）
4. `TASK-417` AI動画段階実装（2026-03-21）
5. `TASK-414` 顧客LTV分析
6. `TASK-412` AIタグ活用導線の改善
7. `TASK-411` 写真カルテのAIタグ付け
8. `TASK-410` LINEの自動マーケ
9. `TASK-409` 事前決済
10. `TASK-408` POS機能導入計画（要件定義〜実装）
11. `TASK-401` 統合会計（Invoice方式）
12. `TASK-POS-001` 要件定義・業務フロー確定（`TASK-408`配下）
13. `TASK-POS-002` データモデル・API契約設計（`TASK-408`配下）
14. `TASK-POS-006` 受入試験・移行・運用ドキュメント整備（`TASK-408`配下）
