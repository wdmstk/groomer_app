...
export function createReservationPaymentToken({
  appointmentId,
  storeId,
  expiresInSeconds = DEFAULT_EXPIRES_IN_SECONDS,
}: {
...
export function verifyReservationPaymentToken(token: string) {
  const secret = getSecret()
  if (!secret) {
    throw new Error('Missing RESERVATION_CANCEL_SECRET or CRON_SECRET_KEY')
  }
  ...
  try {
    payload = JSON.parse(fromBase64Url(payloadBase64)) as PaymentTokenPayload
  } catch {
    return { valid: false as const, reason: 'invalid_payload' as const }
  }
