...
export default async function DevManualSectionPage({ params, searchParams }: DevManualSectionPageProps) {
  ...
  const sequenceIds = activeFlow?.sectionIds ?? devManualSections.map((item) => item.id)
  ...
  const prevHref = prevSectionId ? `/dev/manual/${prevSectionId}${flowQuery}` : null
  const nextHref = nextSectionId ? `/dev/manual/${nextSectionId}${flowQuery}` : null
    ...
    <section className="space-y-4 lg:grid lg:grid-cols-[280px_minmax(0,1fr)] lg:gap-4 lg:space-y-0">
              ...
              >
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {flow.sectionIds.map((id) => {
                    const item = getDevSection(id)
                    if (!item) return null
                    const isActive = item.id === section.id
                    return (
                      <Link
                        key={`${flow.id}-${id}`}
                        href={`/dev/manual/${id}?flow=${flow.id}`}
      ...
      <div className="space-y-4">
        ...
        <p className="text-xs text-gray-500">
          用語の確認が必要な場合は
          <Link href="/dev/manual/glossary" className="ml-1 text-blue-700 hover:underline">
            用語集ページ
          </Link>
          を開いてください。
        </p>
