...
import { applyMenuTemplateDistribution, type MenuTemplateOverwriteScope } from '@/lib/hq/menu-template-distribution'
...
type DeliveryRow = {
  ...
  overwrite_scope: MenuTemplateOverwriteScope
...
async function writeAuditLog(params: {
  ...
  const { error } = await params.supabase.from('audit_logs').insert({
    ...
    actor_scope: 'hq',
    entity_type: 'hq_menu_template_delivery',
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const { error: upsertError } = await supabase.from('hq_menu_template_delivery_approvals').upsert(
    {
      delivery_id,
      store_id: storeId,
  ...
  await writeAuditLog({
    ...
    action: 'hq_menu_template_delivery_approval_recorded',
    before: {
  ...
  if (decision === 'rejected') {
    ...
    await writeAuditLog({
      ...
      actorUserId: user.id,
      entityId: delivery_id,
      action: 'hq_menu_template_delivery_rejected',
      before: {
        status: 'pending',
      },
      after: {
  ...
  const { data: approvalsData, error: approvalsError } = await supabase
    .from('hq_menu_template_delivery_approvals')
  ...
  const approvedStoreIds = Array.from(new Set(approvedRows.map((row) => row.store_id as string)))
  const allApproved = delivery.target_store_ids.every((targetStoreId) => approvedStoreIds.includes(targetStoreId))

  const approvedByUserIds = Array.from(new Set(approvedRows.map((row) => row.approver_user_id as string)))
  ...
  if (!allApproved) {
    ...
    await writeAuditLog({
      ...
      actorUserId: user.id,
      entityId: delivery_id,
      action: 'hq_menu_template_delivery_waiting_remaining_approvals',
      before: {
        status: 'pending',
      },
      after: {
  ...
  const applyResult = await applyMenuTemplateDistribution({
    supabase,
  ...
  if (applyResult.errorMessage) {
    ...
    await writeAuditLog({
      ...
      actorUserId: user.id,
      entityId: delivery_id,
      action: 'hq_menu_template_delivery_apply_failed',
      before: {
        status: 'pending',
      },
      after: {
  ...
  await writeAuditLog({
    ...
    actorUserId: user.id,
    entityId: delivery_id,
    action: 'hq_menu_template_delivery_applied',
    before: {
      status: 'pending',
    },
    after: {
