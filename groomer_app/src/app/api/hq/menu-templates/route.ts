...
import type { MenuTemplateOverwriteScope } from '@/lib/hq/menu-template-distribution'
...
async function resolveManageableStores() {
  ...
  if (candidateStoreIds.length === 0) {
    return {
      error: NextResponse.json(
        { message: 'owner の所属店舗がないため本部テンプレ機能を利用できません。' },
        { status: 403 }
      ),
    }
  }
  ...
  if (manageableStoreIds.length === 0) {
    return {
      error: NextResponse.json(
        { message: 'Proプランの owner 所属店舗がないため本部テンプレ機能を利用できません。' },
        { status: 403 }
      ),
    }
  }
...
export async function POST(request: Request) {
  ...
  await writeAuditLog({
    ...
    action: 'hq_menu_template_delivery_requested',
    before: null,
    after: {
