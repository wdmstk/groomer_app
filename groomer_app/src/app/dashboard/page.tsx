    ...
    return (
      <section className="space-y-4">
        <div className="flex flex-col gap-2">
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-slate-100">ダッシュボード</h1>
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="inline-flex gap-2 rounded-2xl border border-gray-200 bg-white p-2 text-sm dark:border-slate-700 dark:bg-slate-900">
            <Link
              ...
              className={`inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-semibold whitespace-nowrap transition ${
                followupWindowDays === 7 ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'text-gray-600 hover:bg-gray-100 dark:text-slate-300 dark:hover:bg-slate-800'
              }`}
            >
              直近7日
            </Link>
            <Link
              ...
              className={`inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-semibold whitespace-nowrap transition ${
                followupWindowDays === 30 ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'text-gray-600 hover:bg-gray-100 dark:text-slate-300 dark:hover:bg-slate-800'
              }`}
            >
              直近30日
            </Link>
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.3fr_1fr]">
            ...
            <Card>
              ...
              <div className="space-y-3">
                <Link href={buildDashboardHref('followups', followupWindowDays)} className="flex items-center justify-between rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 dark:border-sky-700 dark:bg-sky-950/40"><div><p className="text-sm font-semibold text-sky-900 dark:text-sky-100">来店周期アラート</p><p className="text-xs text-sky-700 dark:text-sky-300">高リスク {highRiskChurnCount} 件 / 未着手 {followupOpenCount} 件</p></div><span className="text-sm font-semibold text-sky-800 dark:text-sky-200">開く</span></Link>
      ...
      {activeTab === 'followups' ? (
        <>
          ...
          <Card>
            <div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-semibold text-gray-900 dark:text-slate-100">再来店フォロー一覧</h2><Link href="/customers/manage?view=alerts" className="text-sm font-semibold text-blue-700 dark:text-blue-300">顧客ペット管理へ</Link></div>
            {followupPriorityRows.length === 0 ? <p className="text-sm text-gray-500 dark:text-slate-400">対応中の再来店フォローはありません。</p> : <div className="space-y-2">{followupPriorityRows.map((task) => (<div key={task.id} className="flex flex-col gap-1 rounded border border-sky-200 bg-sky-50 p-3 text-sm dark:border-sky-700 dark:bg-sky-950/35 md:flex-row md:items-center md:justify-between"><div><p className="font-semibold text-gray-900 dark:text-slate-100">{getRelatedValue(task.customers, 'full_name') ?? '未登録'} / {task.status}</p><p className="text-gray-700 dark:text-slate-300">期限: {task.due_on ?? task.recommended_at.slice(0, 10)} / 電話: {getRelatedValue(task.customers, 'phone_number') ?? '未登録'} / LINE: {getRelatedValue(task.customers, 'line_id') ?? '未登録'}</p></div></div>))}</div>}
          </Card>
          <Card>
            <div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-semibold text-gray-900 dark:text-slate-100">来店周期アラート</h2><p className="text-sm text-gray-500 dark:text-slate-400">全 {revisitAlerts.length} 件</p></div>
            {revisitAlerts.length === 0 ? <p className="text-sm text-gray-500 dark:text-slate-400">来店周期アラートはありません。</p> : <div className="space-y-2">{revisitAlerts.slice(0, 8).map((alert) => (<div key={alert.customerId} className="flex flex-col gap-1 rounded border border-amber-200 bg-amber-50 p-3 text-sm dark:border-amber-700 dark:bg-amber-950/35 md:flex-row md:items-center md:justify-between"><div><p className="font-semibold text-gray-900 dark:text-slate-100">{alert.customerName}（{alert.overdueDays}日超過）</p><p className="text-gray-700 dark:text-slate-300">電話: {alert.phoneNumber ?? '未登録'} / LINE: {alert.lineId ?? '未登録'}</p></div></div>))}<div><Link href="/customers/manage?view=alerts" className="text-sm font-semibold text-blue-700 dark:text-blue-300">顧客ペット管理で一覧を見る</Link></div></div>}
          </Card>
          <Card>
      ...
      {activeTab === 'reoffers' ? (
        <>
          ...
          <Card className={publicReserveLimitedModeOperating ? 'border border-emerald-300 bg-emerald-50 dark:border-emerald-700 dark:bg-emerald-950/30' : 'border border-amber-300 bg-amber-50 dark:border-amber-700 dark:bg-amber-950/30'}>
            <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">空き枠提示型の運用ステータス</p>
