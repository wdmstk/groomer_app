...
export const manualSections: ManualSection[] = [
  ...
  },
  {
    ...
    purpose: 'トリミングメニューとホテルメニューをタブで切り替えて管理します。',
    procedures: [
      '「トリミングメニュー」タブで施術メニューの登録・編集・削除を行います。',
      'ホテルオプション有効時は「ホテルメニュー」タブでホテル商品の登録・編集・削除を行います。',
      '一覧から対象メニューを選択し、必要に応じて更新します。',
    ],
    cautions: [
      '価格・税設定は会計に直結します。',
      'ホテルメニュータブはホテルオプション有効店舗のみ表示されます。',
    ],
  },
  {
    id: 'appointments',
    ...
    purpose: 'トリマー予約とホテル予約を同一画面で切り替え、カレンダーを共通運用します。',
    procedures: [
      'トップのカレンダーで予約状況を確認し、表示フィルターでトリマー/ホテルを切り替えます。',
      '「トリマー予約」で予約の作成・編集・状態更新を行います。',
      'ホテルオプション有効時は「ホテル予約」でホテル予約の一覧・更新を行います。',
    ],
    cautions: [
      '開始/終了日時はJST想定で入力します。',
      'ホテル予約タブはホテルオプション有効店舗のみ表示されます。',
  ...
  },
  {
    ...
    procedures: [
      ...
      '写真を登録し、必要に応じて動画も追加します。',
      '動画タブでプランに応じたAI処理（Assist / Pro / Pro+）を実行します。',
      '処理完了後に要約・提案・ハイライトを確認し、必要に応じてLINE共有します。',
    ],
    cautions: [
      '履歴データのため、記録内容は具体的に入力してください。',
      'Assist / Pro では動画AIは動作しません。動画AIは Pro+ のみです。',
      'AI結果は下書き補助です。確定前にスタッフ確認を行ってください。',
      '処理失敗時は時間を空けて再実行し、継続失敗時は運用担当へ連携してください。',
    ],
  },
  {
    id: 'visits',
  ...
  },
  {
    ...
    title: '会計管理',
    path: '/payments',
    purpose: '統合請求（トリミング + ホテル）を優先して会計確定し、必要時のみ予約単位会計を利用します。',
    procedures: [
      'ホテル台帳で対象滞在を選択し「統合会計を作成」を実行します（必要に応じてトリミング予約と紐付け）。',
      '会計管理の「統合請求（β）」で対象請求IDを開き、支払方法と備考を入力します。',
      '「確定して領収書へ」を実行して1回の会計で確定します。',
      '予約単位会計が必要な場合のみ「予約単位会計（レガシー）」を利用します。',
    ],
    cautions: [
      '統合会計の会計確定には予約ID解決が必要です。ホテル滞在の appointment_id 未設定時は先に紐付けてください。',
      '統合会計に含める予約・滞在は同一顧客である必要があります。',
      '会計確定時に来店履歴が自動作成されます。',
    ],
  },
  {
...
export const manualSectionGuides: Record<string, ManualSectionGuide> = {
  ...
  'medical-records': {
    ...
    itemDetails: [
      ...
      { item: '動画タブ（Assist / Pro / Pro+）', detail: 'Assist は短尺化・要約、Pro は工程推定、Pro+ はハイライト/健康気づきに使います。' },
      { item: 'AI処理の進め方', detail: 'カルテ保存後に実行し、処理完了メッセージを確認してから共有文面を最終調整します。' },
      { item: '失敗時の運用', detail: 'まず再実行し、解消しない場合は運用担当が cron 実行履歴（medical-record-ai-video）を確認します。' },
    ],
  },
  visits: {
  ...
  payments: {
    flow: [
      '利用タイミング: トリミングとホテルの同日会計、またはホテル会計確定時。',
      '前提: ホテル側で統合会計が作成済み、または同一顧客の対象予約・滞在が特定済み。',
      '次に行う操作: 統合請求（β）で会計確定し領収書発行。例外時のみ予約単位会計（レガシー）を利用。',
    ],
    itemDetails: [
      { item: '統合請求（β）パネル', detail: '会計待ち invoice（open/draft）を一覧し、顧客・合計・作成日時を確認します。' },
      { item: '会計確定', detail: '対象請求IDの「会計確定」展開後に支払方法と備考を入力し、「確定して領収書へ」を実行します。' },
      { item: '予約単位会計（レガシー）', detail: '統合請求に載らない個別案件のみ使用します。通常運用では統合請求を優先します。' },
      { item: '会計確定後', detail: 'payments が作成され、領収書表示と来店履歴連携が可能になります。' },
    ],
  },
  receipts: {
  ...
  hotel: {
    flow: [
      '利用タイミング: 預かり予約登録、当日受入、チェックアウト時。',
      '前提: ホテル機能が有効で、対象顧客とペットが登録済み。',
      '次に行う操作: 詳細カードから統合会計を作成し、会計管理で一括会計確定する。',
    ],
    itemDetails: [
      ...
      { item: '統合会計を作成', detail: '選択中滞在の顧客・予約紐付けを使って invoice を生成します。作成後は会計管理へ遷移して会計確定します。' },
    ],
  },
...
export const manualSectionInsights: Record<string, ManualSectionInsight> = {
  ...
  payments: {
    pageGoal: '統合請求を主導線にして会計確定し、トリミングとホテルの同時会計を1回で完了します。',
    tabs: [
      {
        tab: '統合請求（β） / 会計一覧',
        when: 'チェックアウト時 / 施術後 / 締め',
        goal: '統合請求を優先して会計待ちを解消し、必要時のみレガシー会計を使います。',
        cards: [
          {
            card: '統合請求（β）',
            focus: '請求ID、顧客、合計、支払方法、備考。',
            usage: 'ホテル詳細で作成した請求を開き「確定して領収書へ」を実行。',
            decision: '会計確定エラー: ホテル滞在の appointment_id と顧客整合を確認。',
          },
          {
            card: '予約単位会計（レガシー）',
            focus: '統合請求対象外の個別会計処理。',
            usage: '例外案件のみ利用し、通常は統合請求を優先。',
            decision: '統合可能案件: レガシー登録せず統合請求へ戻す。',
          },
        ],
      },
    ],
    ...
    tabs: [
      {
        tab: '新規登録 / 一覧',
        when: '施術直後',
        goal: '観察結果と写真/動画を一貫して記録し、必要なAI補助を使い分けます。',
        cards: [
          {
            card: 'カルテ登録フォーム',
          ...
          {
            card: '動画AI操作（Assist / Pro / Pro+）',
            focus: 'プラン境界、処理状態、結果確認。',
            usage: '保存後に実行し、完了後に要約/提案/ハイライトを確認して共有。',
            decision: '失敗: 再実行。継続失敗: cron 実行履歴を確認して運用連携。',
          },
        ],
      },
    ],
    ...
    tabs: [
      {
        ...
        cards: [
          ...
          {
            card: '統合会計を作成',
            focus: '同一顧客のトリミング予約 + ホテル滞在を1請求へ集約。',
            usage: '請求ID表示後に会計管理を開き、統合請求（β）で会計確定。',
            decision: '作成失敗: 顧客未設定や予約紐付け不足（appointment_id）を補完。',
          },
        ],
      },
    ],
