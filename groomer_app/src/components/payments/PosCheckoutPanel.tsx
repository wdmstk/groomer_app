...
export function PosCheckoutPanel({ customers, products }: PosCheckoutPanelProps) {
    ...
    <div className="rounded border border-amber-200 bg-amber-50 p-4" data-testid="pos-checkout-panel">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-amber-800">店販を含む会計の入力UI。確定APIは次タスクで接続します。</p>
        </div>
      </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto] md:items-end">
        ...
        <div className="text-right text-sm text-slate-700">
          ...
          <Button type="button" className="mt-2" disabled>
            会計確定（API接続待ち）
          </Button>
        </div>
      </div>
