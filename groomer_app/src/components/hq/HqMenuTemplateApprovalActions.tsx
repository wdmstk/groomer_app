...
export function HqMenuTemplateApprovalActions({ deliveries, manageableStoreIds }: Props) {
  ...
  async function sendApproval(deliveryId: string, decision: 'approved' | 'rejected') {
    ...
    try {
      const response = await fetch(`/api/hq/menu-template-deliveries/${deliveryId}/approve`, {
        method: 'POST',
