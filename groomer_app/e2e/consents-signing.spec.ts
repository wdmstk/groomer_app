...
test.describe('電子同意書導線', () => {
  test('店頭署名チャネルで同意書作成メッセージを表示できる', async ({ page }) => {
    ...
    await expect(page.getByTestId('consent-message')).toContainText(
      '同意書を作成しました。署名URL: https://example.com/consent/sign/token-in-person'
    )
  })
  ...
  test('署名画面で署名送信完了とPDFリンクを表示できる', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: '施術前同意書' })).toBeVisible()
