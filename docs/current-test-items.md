...
## 3.1 テスト実装・レビュー時の注意点（運用ルール）

### テストコードの品質
- テストは必ず実際の機能を検証する
- `expect(true).toBe(true)` のような無意味なアサーションは禁止
- 各テストケースで具体的な入力と期待される出力を検証する
- モックは必要最小限にとどめる
...
- 環境変数/設定ファイルでテスト環境と本番環境を分離する

### テスト実装の原則
- Red-Green-Refactor（失敗テストから開始）で実装する
- 境界値・異常系・エラーケースを必ず含める
- カバレッジだけでなく実際の品質を重視する
- テストケース名は検証意図を明確に記述する
...
### テストフレームワーク方針
- 新規/改修 unit test は Vitest を標準とする
- 既存 `node:test` は段階的に Vitest へ移行する
